...
**Workflow**:
1. Launches a Chrome browser (non-headless, maximized)
2. Navigates to https://x.com/login
3. Waits 90 seconds for manual login
4. Saves cookies to `x_cookies.json`
...
Jane Doe,@janedoe,2025-03-25T10:15:00Z,"Solana just hit a new high! $SOL to the moon."
```
...
**Scoring Logic**:
- **Relevance** (1–10): Based on Solana terms + classifier confidence
- **Risk** (1–10): Combines model predictions with scam keyword patterns
...
**Output**:
- `solana_tweets_analyzed.csv`: Adds new columns for each analysis metric
...
**Process**:
1. Reads `solana_tweets_analyzed.csv`
2. Maps sentiment values to numeric scale:
   - Positive: +1
   - Neutral: 0
   - Negative: -1
3. Averages scores per day
4. Converts averages back to sentiment category:
   - > 0.33 → Positive
   - < -0.33 → Negative
   - Otherwise → Neutral
5. Plots and saves `sentiment_momentum_plot.png`
...
#### Step 1: Authentication Setup *(only if `x_cookies.json` fails to log you in during execution of the scrape_solana_logged_in.py script)*
```bash
python save_x_cookies.py
```
...
- `solana_search_tweets.csv`: Raw tweet data
